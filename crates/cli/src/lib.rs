...
impl Install {
    pub fn handle(self) -> CrateResult {
        ...
        let ext_path = build_ext(&artifact, self.release, self.features, self.all_features, self.no_default_features)?;
...
impl Stubs {
    pub fn handle(self) -> CrateResult {
        ...
        let ext_path = if let Some(ext_path) = self.ext {
            ...
            build_ext(&target, false, self.features, self.all_features, self.no_default_features)?.into()
        };
